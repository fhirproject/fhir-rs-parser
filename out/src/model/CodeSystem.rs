...
impl CodeSystem<'_> {
  ...
  pub fn filter(&self) -> Option<Vec<CodeSystem_Filter>> {
    if let Some(Value::Array(val)) = self.value.get("filter") {
      return Some(val.into_iter().map(|e| CodeSystem_Filter { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn supplements(&self) -> Option<String> {
  ...
  pub fn property(&self) -> Option<Vec<CodeSystem_Property>> {
    if let Some(Value::Array(val)) = self.value.get("property") {
      return Some(val.into_iter().map(|e| CodeSystem_Property { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
