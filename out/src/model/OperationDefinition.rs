...
impl OperationDefinition<'_> {
  ...
  pub fn overload(&self) -> Option<Vec<OperationDefinition_Overload>> {
    if let Some(Value::Array(val)) = self.value.get("overload") {
      return Some(val.into_iter().map(|e| OperationDefinition_Overload { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  /// resource.
  pub fn output_profile(&self) -> Option<String> {
  ...
  pub fn resource(&self) -> Option<Vec<String>> {
  ...
  /// covers all the parameters. The profile is a constraint on the parameters
  /// resource as a whole.
  pub fn input_profile(&self) -> Option<String> {
