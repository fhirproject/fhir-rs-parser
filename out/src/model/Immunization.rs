...
impl Immunization<'_> {
  ...
  pub fn reaction(&self) -> Option<Vec<Immunization_Reaction>> {
    if let Some(Value::Array(val)) = self.value.get("reaction") {
      return Some(val.into_iter().map(|e| Immunization_Reaction { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn expiration_date(&self) -> Option<i64> {
    if let Some(val) = self.value.get("expirationDate") {
  ...
  pub fn performer(&self) -> Option<Vec<Immunization_Performer>> {
    if let Some(Value::Array(val)) = self.value.get("performer") {
      return Some(val.into_iter().map(|e| Immunization_Performer { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn education(&self) -> Option<Vec<Immunization_Education>> {
    if let Some(Value::Array(val)) = self.value.get("education") {
      return Some(val.into_iter().map(|e| Immunization_Education { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  /// Date vaccine administered or was to be administered.
  pub fn occurrence_string(&self) -> Option<String> {
  ...
  pub fn protocol_applied(&self) -> Option<Vec<Immunization_ProtocolApplied>> {
    if let Some(Value::Array(val)) = self.value.get("protocolApplied") {
      return Some(val.into_iter().map(|e| Immunization_ProtocolApplied { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
