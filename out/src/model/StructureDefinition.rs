...
impl StructureDefinition<'_> {
  ...
  pub fn context_invariant(&self) -> Option<Vec<String>> {
  ...
  pub fn context(&self) -> Option<Vec<StructureDefinition_Context>> {
    if let Some(Value::Array(val)) = self.value.get("context") {
      return Some(val.into_iter().map(|e| StructureDefinition_Context { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn mapping(&self) -> Option<Vec<StructureDefinition_Mapping>> {
    if let Some(Value::Array(val)) = self.value.get("mapping") {
      return Some(val.into_iter().map(|e| StructureDefinition_Mapping { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn base_definition(&self) -> Option<String> {
