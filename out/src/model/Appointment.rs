...
impl Appointment<'_> {
  ...
  pub fn participant(&self) -> Vec<Appointment_Participant> {
    self.value.get("participant").unwrap().as_array().unwrap().into_iter().map(|e| Appointment_Participant { value: e }).collect::<Vec<_>>()
  }
  ...
  pub fn validate(&self) -> bool {
    ...
    let _ = self.participant().into_iter().for_each(|e| { e.validate(); });
