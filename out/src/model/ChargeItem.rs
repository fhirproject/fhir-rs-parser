...
impl ChargeItem<'_> {
  ...
  pub fn entered_date(&self) -> Option<String> {
  ...
  pub fn performer(&self) -> Option<Vec<ChargeItem_Performer>> {
    if let Some(Value::Array(val)) = self.value.get("performer") {
      return Some(val.into_iter().map(|e| ChargeItem_Performer { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn definition_canonical(&self) -> Option<Vec<String>> {
  ...
  pub fn definition_uri(&self) -> Option<Vec<String>> {
