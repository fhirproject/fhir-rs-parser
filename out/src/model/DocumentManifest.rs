...
impl DocumentManifest<'_> {
  ...
  pub fn content(&self) -> Vec<Reference> {
    self.value.get("content").unwrap().as_array().unwrap().into_iter().map(|e| Reference { value: e }).collect::<Vec<_>>()
  }
  ...
  pub fn related(&self) -> Option<Vec<DocumentManifest_Related>> {
    if let Some(Value::Array(val)) = self.value.get("related") {
      return Some(val.into_iter().map(|e| DocumentManifest_Related { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn validate(&self) -> bool {
    ...
    let _ = self.content().into_iter().for_each(|e| { e.validate(); });
