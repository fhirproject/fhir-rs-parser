...
impl Measure<'_> {
  ...
  pub fn group(&self) -> Option<Vec<Measure_Group>> {
    if let Some(Value::Array(val)) = self.value.get("group") {
      return Some(val.into_iter().map(|e| Measure_Group { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn definition(&self) -> Option<Vec<String>> {
  ...
  pub fn clinical_recommendation_statement(&self) -> Option<String> {
  ...
  /// measure results.
  pub fn risk_adjustment(&self) -> Option<String> {
  ...
  pub fn rate_aggregation(&self) -> Option<String> {
  ...
  pub fn supplemental_data(&self) -> Option<Vec<Measure_SupplementalData>> {
    if let Some(Value::Array(val)) = self.value.get("supplementalData") {
      return Some(val.into_iter().map(|e| Measure_SupplementalData { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
