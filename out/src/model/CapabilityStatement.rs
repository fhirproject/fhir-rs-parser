...
impl CapabilityStatement<'_> {
  ...
  pub fn document(&self) -> Option<Vec<CapabilityStatement_Document>> {
    if let Some(Value::Array(val)) = self.value.get("document") {
      return Some(val.into_iter().map(|e| CapabilityStatement_Document { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  /// types.
  pub fn patch_format(&self) -> Option<Vec<String>> {
  ...
  pub fn rest(&self) -> Option<Vec<CapabilityStatement_Rest>> {
    if let Some(Value::Array(val)) = self.value.get("rest") {
      return Some(val.into_iter().map(|e| CapabilityStatement_Rest { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  /// their entirety.
  pub fn implementation_guide(&self) -> Option<Vec<String>> {
  ...
  pub fn messaging(&self) -> Option<Vec<CapabilityStatement_Messaging>> {
    if let Some(Value::Array(val)) = self.value.get("messaging") {
      return Some(val.into_iter().map(|e| CapabilityStatement_Messaging { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
