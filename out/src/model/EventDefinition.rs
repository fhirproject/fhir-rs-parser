...
impl EventDefinition<'_> {
  ...
  pub fn trigger(&self) -> Vec<TriggerDefinition> {
    self.value.get("trigger").unwrap().as_array().unwrap().into_iter().map(|e| TriggerDefinition { value: e }).collect::<Vec<_>>()
  }
  ...
  pub fn validate(&self) -> bool {
    ...
    let _ = self.trigger().into_iter().for_each(|e| { e.validate(); });
