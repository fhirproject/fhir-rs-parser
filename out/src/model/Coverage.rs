...
impl Coverage<'_> {
  ...
  pub fn dependent(&self) -> Option<String> {
  ...
  pub fn cost_to_beneficiary(&self) -> Option<Vec<Coverage_CostToBeneficiary>> {
    if let Some(Value::Array(val)) = self.value.get("costToBeneficiary") {
      return Some(val.into_iter().map(|e| Coverage_CostToBeneficiary { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn payor(&self) -> Vec<Reference> {
    self.value.get("payor").unwrap().as_array().unwrap().into_iter().map(|e| Reference { value: e }).collect::<Vec<_>>()
  }
  ...
  pub fn subscriber_id(&self) -> Option<String> {
  ...
  pub fn class(&self) -> Option<Vec<Coverage_Class>> {
    if let Some(Value::Array(val)) = self.value.get("class") {
      return Some(val.into_iter().map(|e| Coverage_Class { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn validate(&self) -> bool {
    ...
    let _ = self.payor().into_iter().for_each(|e| { e.validate(); });
