...
impl Claim<'_> {
    ...
    pub fn supporting_info(&self) -> Option<Vec<Claim_SupportingInfo>> {
        if let Some(Value::Array(val)) = self.value.get("supportingInfo") {
            return Some(
                val.into_iter()
                    .map(|e| Claim_SupportingInfo { value: e })
                    .collect::<Vec<_>>(),
            );
        }
        return None;
