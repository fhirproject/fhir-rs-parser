...
impl Contract<'_> {
  ...
  pub fn friendly(&self) -> Option<Vec<Contract_Friendly>> {
    if let Some(Value::Array(val)) = self.value.get("friendly") {
      return Some(val.into_iter().map(|e| Contract_Friendly { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn rule(&self) -> Option<Vec<Contract_Rule>> {
    if let Some(Value::Array(val)) = self.value.get("rule") {
      return Some(val.into_iter().map(|e| Contract_Rule { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn signer(&self) -> Option<Vec<Contract_Signer>> {
    if let Some(Value::Array(val)) = self.value.get("signer") {
      return Some(val.into_iter().map(|e| Contract_Signer { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn legal(&self) -> Option<Vec<Contract_Legal>> {
    if let Some(Value::Array(val)) = self.value.get("legal") {
      return Some(val.into_iter().map(|e| Contract_Legal { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn instantiates_uri(&self) -> Option<String> {
