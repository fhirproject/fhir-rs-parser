...
impl Invoice<'_> {
  ...
  pub fn participant(&self) -> Option<Vec<Invoice_Participant>> {
    if let Some(Value::Array(val)) = self.value.get("participant") {
      return Some(val.into_iter().map(|e| Invoice_Participant { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  /// of payment.
  pub fn payment_terms(&self) -> Option<String> {
  ...
  pub fn line_item(&self) -> Option<Vec<Invoice_LineItem>> {
    if let Some(Value::Array(val)) = self.value.get("lineItem") {
      return Some(val.into_iter().map(|e| Invoice_LineItem { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
