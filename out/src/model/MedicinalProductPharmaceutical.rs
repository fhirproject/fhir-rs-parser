...
impl MedicinalProductPharmaceutical<'_> {
  ...
  pub fn administrable_dose_form(&self) -> CodeableConcept {
    CodeableConcept {
      value: &self.value["administrableDoseForm"],
    }
  }
  ...
  pub fn characteristics(&self) -> Option<Vec<MedicinalProductPharmaceutical_Characteristics>> {
    if let Some(Value::Array(val)) = self.value.get("characteristics") {
      return Some(val.into_iter().map(|e| MedicinalProductPharmaceutical_Characteristics { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  /// the body.
  pub fn route_of_administration(&self) -> Vec<MedicinalProductPharmaceutical_RouteOfAdministration> {
    self.value.get("routeOfAdministration").unwrap().as_array().unwrap().into_iter().map(|e| MedicinalProductPharmaceutical_RouteOfAdministration { value: e }).collect::<Vec<_>>()
  }
  ...
  pub fn validate(&self) -> bool {
    ...
    let _ = self.administrable_dose_form().validate();
    ...
    let _ = self.route_of_administration().into_iter().for_each(|e| { e.validate(); });
