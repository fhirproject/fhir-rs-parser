...
impl MedicinalProductPharmaceutical<'_> {
    ...
    pub fn characteristics(&self) -> Option<Vec<MedicinalProductPharmaceutical_Characteristics>> {
        if let Some(Value::Array(val)) = self.value.get("characteristics") {
            return Some(
                val.into_iter()
                    .map(|e| MedicinalProductPharmaceutical_Characteristics { value: e })
                    .collect::<Vec<_>>(),
            );
        }
        return None;
    ...
    pub fn route_of_administration(
        ...
        self.value
            ...
            .unwrap()
            .into_iter()
            .map(|e| MedicinalProductPharmaceutical_RouteOfAdministration { value: e })
            .collect::<Vec<_>>()
    }
    ...
    pub fn validate(&self) -> bool {
        ...
        let _ = self.route_of_administration().into_iter().for_each(|e| {
            e.validate();
        });
