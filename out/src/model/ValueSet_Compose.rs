...
impl ValueSet_Compose<'_> {
  ...
  pub fn exclude(&self) -> Option<Vec<ValueSet_Include>> {
    if let Some(Value::Array(val)) = self.value.get("exclude") {
      return Some(val.into_iter().map(|e| ValueSet_Include { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn locked_date(&self) -> Option<String> {
  ...
  pub fn include(&self) -> Vec<ValueSet_Include> {
    self.value.get("include").unwrap().as_array().unwrap().into_iter().map(|e| ValueSet_Include { value: e }).collect::<Vec<_>>()
  }
  ...
  pub fn validate(&self) -> bool {
    ...
    let _ = self.include().into_iter().for_each(|e| { e.validate(); });
    return true;
  }
