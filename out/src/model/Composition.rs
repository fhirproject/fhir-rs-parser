...
impl Composition<'_> {
  ...
  pub fn author(&self) -> Vec<Reference> {
    self.value.get("author").unwrap().as_array().unwrap().into_iter().map(|e| Reference { value: e }).collect::<Vec<_>>()
  }
  ...
  pub fn event(&self) -> Option<Vec<Composition_Event>> {
    if let Some(Value::Array(val)) = self.value.get("event") {
      return Some(val.into_iter().map(|e| Composition_Event { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn attester(&self) -> Option<Vec<Composition_Attester>> {
    if let Some(Value::Array(val)) = self.value.get("attester") {
      return Some(val.into_iter().map(|e| Composition_Attester { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn confidentiality(&self) -> Option<String> {
  ...
  pub fn relates_to(&self) -> Option<Vec<Composition_RelatesTo>> {
    if let Some(Value::Array(val)) = self.value.get("relatesTo") {
      return Some(val.into_iter().map(|e| Composition_RelatesTo { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn validate(&self) -> bool {
    ...
    let _ = self.author().into_iter().for_each(|e| { e.validate(); });
