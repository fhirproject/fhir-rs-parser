...
impl RiskEvidenceSynthesis_Certainty<'_> {
  ...
  pub fn certainty_subcomponent(&self) -> Option<Vec<RiskEvidenceSynthesis_CertaintySubcomponent>> {
    if let Some(Value::Array(val)) = self.value.get("certaintySubcomponent") {
      return Some(val.into_iter().map(|e| RiskEvidenceSynthesis_CertaintySubcomponent { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
