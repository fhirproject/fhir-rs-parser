...
impl Task<'_> {
  ...
  pub fn input(&self) -> Option<Vec<Task_Input>> {
    if let Some(Value::Array(val)) = self.value.get("input") {
      return Some(val.into_iter().map(|e| Task_Input { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn output(&self) -> Option<Vec<Task_Output>> {
    if let Some(Value::Array(val)) = self.value.get("output") {
      return Some(val.into_iter().map(|e| Task_Output { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn instantiates_canonical(&self) -> Option<String> {
