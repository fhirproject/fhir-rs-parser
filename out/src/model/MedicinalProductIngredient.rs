...
impl MedicinalProductIngredient<'_> {
  ...
  pub fn specified_substance(&self) -> Option<Vec<MedicinalProductIngredient_SpecifiedSubstance>> {
    if let Some(Value::Array(val)) = self.value.get("specifiedSubstance") {
      return Some(val.into_iter().map(|e| MedicinalProductIngredient_SpecifiedSubstance { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
