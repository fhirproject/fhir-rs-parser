...
impl Provenance<'_> {
  ...
  pub fn agent(&self) -> Vec<Provenance_Agent> {
    self.value.get("agent").unwrap().as_array().unwrap().into_iter().map(|e| Provenance_Agent { value: e }).collect::<Vec<_>>()
  }
  ...
  pub fn signature(&self) -> Option<Vec<Signature>> {
    if let Some(Value::Array(val)) = self.value.get("signature") {
      return Some(val.into_iter().map(|e| Signature { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  pub fn entity(&self) -> Option<Vec<Provenance_Entity>> {
    if let Some(Value::Array(val)) = self.value.get("entity") {
      return Some(val.into_iter().map(|e| Provenance_Entity { value: e }).collect::<Vec<_>>());
    }
    return None;
  }
  ...
  /// The period during which the activity occurred.
  pub fn occurred_date_time(&self) -> Option<String> {
  ...
  pub fn target(&self) -> Vec<Reference> {
    self.value.get("target").unwrap().as_array().unwrap().into_iter().map(|e| Reference { value: e }).collect::<Vec<_>>()
  }
  ...
  pub fn validate(&self) -> bool {
    ...
    let _ = self.target().into_iter().for_each(|e| { e.validate(); });
    return true;
  }
